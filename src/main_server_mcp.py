...
mcp = FastMCP(
    ...
    capabilities={
        ...
        "resources": True, # We don't use resources
        "prompts": True   # We don't use prompts
    }
)

# Connection string from --conn flag or POSTGRES_CONNECTION_STRING env var
parser = argparse.ArgumentParser(description="C2S Veiculo MCP server")
parser.add_argument(
    "--conn",
    dest="conn",
...
logger.info(
    "Starting MCP server – connection Postgresql %s",
...
@mcp.tool()
def lista_todos_veiculos() -> str:
    
    """Lista todos veiculos da tabelas sem nenhum parâmetro.
    
    Args:
        nenhuma
    """
    logger.info("lista de todos os veiculos cadatrados")
    sql = """
    SELECT
    v.numero_chassi,
    v.cor,
    ...
    FROM Veiculo AS v
    JOIN Modelo AS mo ON v.Modelo_idModelo = mo.idModelo
    JOIN Marca AS ma ON mo.Marca_idMarca = ma.idMarca
    """
    return query(sql)
...
@mcp.tool()
def lista_veiculos_ano_fabricacao(ano_fabricacao: str = None) -> str:
    
    """Lista todos veiculos da tabelas com ano_fabricacao como parâmetro.
    
    Args:
        ano_fabricacao: ano de fabricação do veiculo
    """
    logger.info("lista de veiculos com ano_fabricacao como parâmetro")
    if not ano_fabricacao:
        return {"erro": "ano_fabricacao não fornecido", "veiculos": []}
    sql = """
    SELECT
    v.numero_chassi,
    v.cor,
    ...
    JOIN Modelo AS mo ON v.Modelo_idModelo = mo.idModelo
    JOIN Marca AS ma ON mo.Marca_idMarca = ma.idMarca
    WHERE v.ano_fabricacao = %s
    """
    return query(sql, [ano_fabricacao])

@mcp.tool()
def lista_veiculos_numero_chassi(numero_chassi: str = None) -> str:
    """Lista todos veiculos da tabelas com numero_chassi como parâmetro.
    
    Args:
        numero_chassi: numero do cahssi do carro
    """
    logger.info("lista de veiculos com numero_chassi como parâmetro")
    ...
    JOIN Modelo AS mo ON v.Modelo_idModelo = mo.idModelo
    JOIN Marca AS ma ON mo.Marca_idMarca = ma.idMarca
    WHERE v.numero_chassi = %s
    """
    return query(sql, [numero_chassi])

@mcp.tool()
def lista_veiculos_cor(cor: str = None) -> dict:
    """Lista todos veiculos da tabelas com cor como parâmetro.
    
    Args:
        cor: nome da cor
    """
    logger.info("lista de veiculos com cor como parâmetro")
    sql = """
    SELECT
    v.numero_chassi,
    v.cor,
    ...
    WHERE v.cor = %s
    """
    return query(sql, [cor])

@mcp.tool()
def lista_veiculos_marca(marca: str = None) -> str:
    """Lista todos veiculos da tabelas com marca como parâmetro.
    
    Args:
        marca: nome da marca
    """
    logger.info("lista de veiculos com marca como parâmetro")
    sql = """
    SELECT
    v.numero_chassi,
    v.cor,
    ...
    WHERE ma.nome = %s
    """
    return query(sql, [marca])


@mcp.tool()
def lista_veiculos_modelo(modelo: str = None) -> str:
    """Lista todos veiculos da tabelas com mmodelo como parâmetro.
    
    Args:
        modelo: nome do modelo
    """
    logger.info("lista de veiculos com modelo como parâmetro")
    sql = """
    SELECT
    v.numero_chassi,
    v.cor,
    ...
    WHERE mo.nome = %s
    """
    return query(sql, [modelo])


@mcp.tool()
def lista_veiculos_marca_modelo(marca: str = None,modelo: str = None) -> str:
    """Lista todos veiculos da tabelas com marca e modelo como parâmetro.
    
    Args:
        marca: nome da marca
        modelo: nome do modelo
    """
    logger.info("lista de veiculos com marca e modelo como parâmetro")
    sql = """
...
if __name__ == "__main__":
    try:
        logger.info("Starting MCP Veiculo server...")
        mcp.run()
    except Exception as e:
